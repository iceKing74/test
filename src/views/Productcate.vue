  ...
  <div class="xx-panel">
    ...
    <div class="heading">
      ...
      <el-breadcrumb>
        ...
        <el-breadcrumb-item to="/control/product">产品列表管理</el-breadcrumb-item>
    ...
    <div class="datagrid">
      ...
      >
        ...
        :width="80"
        sortable />
        <el-table-column label="分类图片" v-slot="scope">
          <el-image
        ...
        <el-table-column  label="类别名称" prop="title" />
...
export default {
  data () {
    return {
      ...
      rules: {
        ...
        thumb: { required: true, message: '必须存在产品列表图片' }
      }
    }
  },
  watch: {
  ...
  methods: {
    query () {
      this.$http.post('productcate/list', this.condition).then(res => {
        if (res.err > 0) {
    ...
    handleSwitchChange ({ id, state }) {
      this.$http.patch('productcate/state', { id, state }).then(res => {
        if (res.err > 0) {
          this.$message.error(res.desc)
        }
      })
    ...
    handleRemove (ids) {
      this.$confirm('是否删除数据', '提示').then(_ => {
        this.$http.post('productcate/remove', JSON.stringify({ ids })).then(res => {
          if (res.err > 0) {
            this.$message.error(res.desc)
          } else {
            this.query()
